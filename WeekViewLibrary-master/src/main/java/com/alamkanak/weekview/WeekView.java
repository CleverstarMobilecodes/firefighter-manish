...
//            boolean sameDay = isSameDay(day, today);
    ...
    private void drawEventTitle(WeekViewEvent event, RectF rect, Canvas canvas, float originalTop, float originalLeft) {
        ...
        if (event.getLocation() != null) {
            ...
            String white = event.getLocation();
            SpannableString redSpannable = new SpannableString(white);
            redSpannable.setSpan(new ForegroundColorSpan(Color.WHITE), 0, white.length(), 0);
            spannableStringBuilder.append(redSpannable);
        }
        ...
        // Get text dimensions.
        StaticLayout textLayout = new StaticLayout(spannableStringBuilder, mEventTextPaint, availableWidth, Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
            ...
            if (availableHeight >= lineHeight) {
                ...
                do {
                    ...
                    if (event.getId() != mNewEventId)
                        textLayout = new StaticLayout(TextUtils.ellipsize(spannableStringBuilder, mEventTextPaint, availableLineCount * availableWidth, TextUtils.TruncateAt.END), mEventTextPaint, (int) (rect.right - originalLeft - mEventPadding * 2), Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
